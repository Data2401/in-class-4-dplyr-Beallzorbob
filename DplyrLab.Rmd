...
average_height <- starwars %>% group_by(eye_color) %>% summarize(mean(height))
```
...
```{r}
starwars %>% group_by(eye_color) %>% summarize(mean(height)) %>% filter(eye_color == "brown")
...
```{r}
starwars %>% group_by(eye_color) %>% summarize(mean(na.omit(height)))
...
```{r}
starwars %>% filter(sex == "male") %>% arrange(desc(height))
